...
class PDFReportService:
    ...
    def _create_header(self, config: ReportConfig) -> List:
        ...
        meta_data.append(f"<b>Date:</b> {datetime.now(timezone.utc).strftime('%B %d, %Y')}")
    ...
    def _create_footer(self, canvas, doc):
        ...
        # Generated by
        canvas.drawString(0.5*inch, 0.5*inch, f"Generated by AgTools | {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')}")
    ...
    def generate_scouting_report(
        ...
        field_data = [
            ...
            ["Scout Date", datetime.now(timezone.utc).strftime('%B %d, %Y')],
        ]
    ...
    def generate_equipment_status_report(
        ...
        if config is None:
            config = ReportConfig(
                ...
                subtitle=f"As of {datetime.now(timezone.utc).strftime('%B %d, %Y')}"
            )
    ...
    def generate_dashboard_summary_pdf(
        ...
        elements.append(Spacer(1, 30))
        elements.append(Paragraph(
            f"<i>Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}</i>",
            self.styles['Normal']
        ))
